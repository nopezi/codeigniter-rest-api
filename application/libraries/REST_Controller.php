...
abstract class REST_Controller extends CI_Controller {
    ...
    protected function _detect_api_key()
    {
        ...
        $this->rest->pengguna_app_id = NULL;
        ...
        if (($key = isset($this->_args[$api_key_variable]) ? $this->_args[$api_key_variable] : $this->input->server($key_name)))
        {
            ...
            isset($row->pengguna_app_id) && $this->rest->pengguna_app_id = $row->pengguna_app_id;
            isset($row->level) && $this->rest->level = $row->level;
            isset($row->ignore_limits) && $this->rest->ignore_limits = $row->ignore_limits;
            
            $this->_apiuser = $row;

            //custom check. Api_key and id_pengguna_app must match
            // $id_pengguna_app = isset($this->_args['id_pengguna_app'])?$this->_args['id_pengguna_app'] : null;
            // if (!$id_pengguna_app){
            //     $this->response([
            //         $this->config->item('rest_status_field_name') => FALSE,
            //         $this->config->item('rest_message_field_name') => "id_pengguna_app not found!"
            //     ], self::HTTP_FORBIDDEN);
            // }
            // if($row->pengguna_app_id != $this->_args['id_pengguna_app']){
            //     return FALSE;
            // }
    ...
    protected function _check_login($username = NULL, $password = FALSE)
    {
        ...
        //Custom. set rest_valid_logins, value from databse
        // $this->load->model('Model_pengguna_app');
        // $valid_login_db = $this->Model_pengguna_app->get_valid_login();
        // $valid_logins = $valid_login_db;
        

        // //custom check_login, using bycript
        // $data_login = array(
        //     'username' => $username,
        //     'password' => $password
        ...
        // );
        // $this->load->model('Model_pengguna_app');
        // $result = $this->Model_pengguna_app->login($data_login);
        // if($result){
        //     return TRUE;
        // }
        // ====================================
